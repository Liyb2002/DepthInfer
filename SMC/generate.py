...
class generate_helper:
    ...
    def smc_process(self):
        ...
        self.procedural_generate(start_type, start_pos, connected_dir)
        

        return self.particle_list[0]


    def procedural_generate(self, start_type, startPos, connected_dir):

        for i in range(len(self.particle_list)):
            tempt_particle = self.particle_list[i]
            tempt_particle.prepare_particle(startPos, start_type, connected_dir)
